...
int __init dLinkedListTest_init(void){
	...
	kthread_run((void*)threadTestInsert,NULL,"test_thread");
    }
    ...
    INIT_LIST_HEAD(&spin_list);
    INIT_LIST_HEAD(&mutex_list);
    INIT_LIST_HEAD(&rw_list);
    
    struct my_node* initial_list = kmalloc(sizeof(struct my_node), GFP_KERNEL);
    initial_list->data = 1;
    list_add(&initial_list->list, &spin_list);

    struct my_node * initial_list2 = kmalloc(sizeof(struct my_node), GFP_KERNEL);
    initial_list2->data = 1;
    list_add(&initial_list2->list, &mutex_list);

    struct my_node * initial_list3 = kmalloc(sizeof(struct my_node), GFP_KERNEL);
    initial_list3 ->data = 1;
    list_add(&initial_list3->list, &rw_list);
   
    getnstimeofday(&starttime);
    start = (unsigned long)starttime.tv_sec*1000000000 + (unsigned long)starttime.tv_nsec;
    for(i =0; i<THREAD_COUNT;i++){
	kthread_run((void*)threadTestSpin,NULL,"test_thread");
    }

    ssleep(10);
    printk("Spinlock linked list Insert Time: %lld ns", end - start);
    ...
    start = (unsigned long)starttime.tv_sec*1000000000 + (unsigned long)starttime.tv_nsec;
    for(i =0; i<THREAD_COUNT;i++){
	kthread_run((void*)threadTestMutex,NULL,"test_thread");
    }

    ssleep(10);
    printk("Mutex Linked List Insert Time: %lld ns", end - start);
 

    getnstimeofday(&starttime);
    start = (unsigned long)starttime.tv_sec*1000000000 + (unsigned long)starttime.tv_nsec;
    for(i =0; i<THREAD_COUNT;i++){
	kthread_run((void*)threadTestSema,NULL,"test_thread");
    }

    ssleep(10);
    printk("Semaphore Linked List Insert Time: %lld ns", end - start);
