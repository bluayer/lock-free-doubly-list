...
void threadTest1(){
    ...
    {
	Node *p=(Node *)malloc(sizeof(struct Node));//new Node;
	assert(p);
	p->data=i;
	do
	{
...
int main (void){
	...
	treadCreated=pthread_create(&p_thread[i],NULL,(void*)threadTest1,NULL);
	if(treadCreated!=0)
	{
    ...
    sleep(2);// 스레드 싱크 맞추기 (여기에 세마포어 걸어두시면 될것같아여
    printf("First for loop finish\n");
    ...
    for(int i =0; i<THREAD_COUNT;i++){
	treadCreated=pthread_create(&p_thread[i],NULL,(void*)threadTest2,NULL);
	if(treadCreated!=0)
	{
	    printf("thread create fail\n");
	}
